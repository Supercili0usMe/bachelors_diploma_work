...
class Solution:
    ...
    def maximumSafenessFactor(self, grid: List[List[int]]) -> int:
        n = len(grid)
        multi_score_queue = deque()
        for i in range(n):
            for j in range(n):
                if grid[i][j] == 1:
                    multi_score_queue.append((i, j))
                    grid[i][j] = 0
                else:
                    grid[i][j] = -1
        
        while multi_score_queue:
            size = len(multi_score_queue)
            while size > 0:
                curr = multi_score_queue.popleft()
                for d in self.dir:
                    di, dj = curr[0] + d[0], curr[1] + d[1]
                    val = grid[curr[0]][curr[1]]
                    if self.isValidCell(grid, di, dj) and grid[i][j] == -1:
                        grid[di][dj] = val + 1
                        multi_score_queue.append((di, dj))
                size -= 1
        
        start, stop, res = 0, 0, -1
        for i in range(n):
            for j in range(n):
